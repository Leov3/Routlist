...
export default function MaintenancePage() {
  ...
  async function load(showSpinner = false) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "No se pudo cargar el módulo de mantenimiento.");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    try {
      await api("/maintenance/backups", {
        ...
        body: JSON.stringify({
          ...
          includeDatabase:
            manualScope === "both" || manualScope === "db",
          includeStorage:
            manualScope === "both" || manualScope === "storage",
        }),
      });
  ...
  async function deleteBackup(id: string) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "No se pudo eliminar el backup.");
    } finally {
      setDeletingId(null);
    }
  }
  ...
  return (
    <ProtectedPage allowedRoles={["OWNER"]}>
      ...
      <section className="mb-5 grid gap-3 md:grid-cols-4">
        <SummaryCard label="Migraciones aplicadas" value={migration?.appliedCount ?? 0} icon={Database} />
        <SummaryCard label="Pendientes" value={migration?.pendingCount ?? 0} icon={AlertTriangle} />
        <SummaryCard label="Backups" value={backupTotals.total} icon={Archive} />
        <SummaryCard label="Errores" value={backupTotals.failed} icon={ShieldAlert} />
      </section>
        ...
        <div className="space-y-5">
          <div className="rounded-[28px] border border-outline-variant bg-surface-container p-5 shadow-elevation-1">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <InfoPill label="Última aplicada" value={migration?.lastAppliedAt ?? "N/D"} />
              <InfoPill label="Pendientes" value={String(migration?.pendingCount ?? 0)} />
              <InfoPill label="Ruta backups" value={status?.storage.backupRootPath ?? "N/D"} />
              <InfoPill
            ...
            <div className="mt-4 rounded-2xl border border-outline-variant bg-surface px-4 py-3 text-sm text-on-surface-variant">
              Se conservan solo las 3 copias más recientes de base de datos en el servidor. Los backups
          ...
          <div className="rounded-[28px] border border-outline-variant bg-surface-container p-5 shadow-elevation-1">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={() => setManualLabel(`Respaldo ${new Date().toLocaleString()}`)}
          ...
          <div className="rounded-[28px] border border-outline-variant bg-surface-container p-5 shadow-elevation-1">
            ...
            <p className="mt-1 text-sm text-on-surface-variant">
              {backups.length} backups guardados en total. Disponible para descarga y restauración.
            </p>
                  ...
                  >
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-sm text-on-surface-variant">
                          {backup.source} · {formatBytes(backup.sizeBytes)} · {backup.createdBy?.fullName ?? "Sistema"}
                        </p>
                        <p className="mt-1 text-xs text-on-surface-variant">
                          {backup.createdAt}
                        </p>
                        {backup.errorMessage && (
                          <p className="mt-2 text-sm text-red-300">{backup.errorMessage}</p>
                        )}
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {backup.databaseDumpFileName && backup.databaseDownloadUrl && (
                          <a
                        ...
                        {backup.includeStorage && backup.storageDownloadUrl && (
                          <a
                        ...
                        <button
                          type="button"
                          disabled={restoringId === backup.id || backup.status !== "COMPLETED"}
        ...
        <aside className="space-y-5">
          <div className="rounded-[28px] border border-outline-variant bg-surface-container p-5 shadow-elevation-1">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-on-surface-variant">
                  Define si el módulo opera de forma programada o solo manual.
                </p>
              </div>
              <span
            ...
            <div className="mt-4 grid gap-3">
              ...
              <ToggleRow
                ...
                onChange={(checked) =>
                  setForm((current) => ({ ...current, includeDatabase: checked }))
                }
              />
              <ToggleRow
                ...
                onChange={(checked) =>
                  setForm((current) => ({ ...current, includeStorage: checked }))
                }
              />
              ...
              <label className="space-y-2">
                ...
                <select
                  ...
                  onChange={(event) =>
                    setForm((current) => ({
                      ...current,
                      scheduleMode: event.target.value as SettingsForm["scheduleMode"],
                    }))
                  }
                  className="h-11 w-full rounded-2xl border border-outline-variant bg-surface px-4 text-sm text-on-surface outline-none transition-colors focus:border-primary"
                >
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      setForm((current) => ({ ...current, everyHours: event.target.value }))
                    }
                    className="h-11 w-full rounded-2xl border border-outline-variant bg-surface px-4 text-sm text-on-surface outline-none transition-colors focus:border-primary"
                  />
                </label>
          ...
          <div className="rounded-[28px] border border-outline-variant bg-surface-container p-5 shadow-elevation-1">
            ...
            <div className="mt-4 grid gap-3">
              {audit.length ? (
                audit.map((item) => (
                  <div key={item.id} className="rounded-2xl border border-outline-variant bg-surface px-4 py-3">
    ...
    <div className="rounded-[24px] border border-outline-variant bg-surface-container p-4 shadow-elevation-1">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-xl font-semibold tracking-tight text-on-surface">{value}</p>
        </div>
      </div>
    </div>
  );
